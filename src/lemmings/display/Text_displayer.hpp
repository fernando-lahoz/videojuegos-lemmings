...
public:
  Text_displayer(Point3f position, Vector2f letter_size, Game_info &_game_info, std::string _justified, const Texture &font_texture,
      ...
      : FixedText(_justified == "center" ? Point3f(position.x - letter_size.x * text.size() / 2, position.y - letter_size.y / 2, position.z) : position, letter_size, font_texture, texture_letter_size, map, text, engine, _class_name),
