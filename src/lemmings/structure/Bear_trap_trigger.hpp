...
class Bear_trap_trigger : public Trigger
{
  bool change = false;
  bool init = false;
  bool is_debug = false;
...
public:
  ...
  Bear_trap_trigger(Point3f position, Vector2f diagonal, Engine &engine, Structure &_trap, bool debug = false) : Trigger(position, diagonal, engine, "BEAR TRAP TRIGGER"), trap(_trap)
  {
    init = !debug;
    is_debug = debug;
  }
