...
class Tons_trigger : public Trigger
{
  bool change = false;
  bool init = false;
  bool is_debug = false;
...
public:
  ...
  Tons_trigger(Point3f position, Vector2f diagonal, Engine &engine, Structure &_trap, bool debug = false) : Trigger(position, diagonal, engine, "TONS TRIGGER"), trap(_trap)
  {
    init = !debug;
    is_debug = debug;
  }
