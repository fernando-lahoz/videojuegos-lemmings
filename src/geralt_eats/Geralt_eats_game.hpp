...
public:
    ...
    void write_alpha_to_file(Texture txt)
    {
        ...
        auto alpha_mask = *txt.get_alpha_mask();

        //std::cout << alpha_mask << std::endl;
        std::ofstream file("alpha_mask.txt");
        for (size_t i = 0; i < alpha_mask.size(); i++)
        {
            if (i % txt.get_alpha_mask_width() == 0)
                file << std::endl;

            file << (unsigned int)alpha_mask[i] << " ";
        }
        file.close();
    }
    ...
    void on_game_startup(Engine &engine) override
    {
        ...
        //auto test = std::make_shared<Test_obj>(Point2f(0.5, 0.5), Vector2f(0.2, 0.2), t3, engine);


        write_alpha_to_file(wall_txt);

        auto cielo = std::make_shared<Entity>(Point2f(-10, -5), 10, Vector2f(20, 11.2), cielo_txt, engine, "Cielo", Entity::Physics_type::NONE, Entity::Collision_check::NONE, Entity::Collision_type::STATIC_BODY, Entity::Cursor_type::AABB, "Cielo");
        ...
        create_camera(std::make_shared<Geralt_camera>(geralt));
    }
